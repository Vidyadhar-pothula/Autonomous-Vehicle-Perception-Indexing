...
class Car:
    ...
    def emergency_brake(self, dt: float = 0.1) -> None:
        """Emergency braking"""
        self.speed = max(0, self.speed - self.deceleration * dt * 3.6 * 2.0)


class SensorDataProcessor:
    """Processes B-tree sensor data for decision making"""
    ...
    def get_surrounding_cars(self, our_car: Car, all_cars: List[Car]) -> Dict:
        """Get surrounding cars using B-tree sensor data"""
        surrounding = {
            'ahead_same_lane': [],
            'behind_same_lane': [],
            'left_lane': [],
        ...
        recent_keys = self.btree_manager.inorder_keys()[-50:]  # Last 50 readings
        ...
        for key in recent_keys:
            ...
            # Only consider cars within 200m
            if abs(distance) < 200:
                car_data = {
...
class AutonomousDriver:
    """Professional autonomous driving decision system"""
    ...
    def _check_emergency(self, our_car: Car, surrounding: Dict) -> Optional[str]:
        ...
        for car in surrounding['ahead_same_lane']:
            ...
            elif car['distance'] < 40 and car['relative_speed'] > 10:  # Approaching fast
                return "Emergency Braking"
    ...
    def _check_overtaking_opportunity(self, our_car: Car, surrounding: Dict) -> Optional[str]:
        ...
        # Only overtake if car is significantly slower
        if slowest_ahead and slowest_ahead['speed'] < our_car.speed * 0.7:
            # Check if left lane is available (preferred for overtaking)
            if our_car.lane_id > 1 and self.sensor_processor.is_lane_clear(our_car.lane_id - 1, our_car, 40):
                self.overtaking_state = 'changing_left'
                self.overtaking_timer = 0
                self.target_car_id = slowest_ahead['car_id']
                return "Initiating Overtake - Left Lane"
            # Check right lane as alternative
            elif our_car.lane_id < 3 and self.sensor_processor.is_lane_clear(our_car.lane_id + 1, our_car, 40):
                self.overtaking_state = 'changing_right'
                self.overtaking_timer = 0
                self.target_car_id = slowest_ahead['car_id']
                return "Initiating Overtake - Right Lane"
        
        return None
    
    def _handle_overtaking(self, our_car: Car, surrounding: Dict) -> str:
        """Handle ongoing overtaking maneuver"""
        self.overtaking_timer += 1
        
        if self.overtaking_state == 'changing_left':
            if self.overtaking_timer >= 3:  # 3 ticks to change lanes
                our_car.lane_id -= 1
            ...
            if self.overtaking_timer >= 5:  # 5 ticks to accelerate
            ...
            if target_passed or self.overtaking_timer >= 15:  # Timeout after 15 ticks
                # Determine original lane
                original_lane = 2 if our_car.lane_id != 2 else 1  # Default to lane 2 or 1
                if self.sensor_processor.is_lane_clear(original_lane, our_car, 30):
                    our_car.lane_id = original_lane
                    self.overtaking_state = None
                    self.target_car_id = None
            ...
            original_lane = 2 if our_car.lane_id != 2 else 1
            if self.sensor_processor.is_lane_clear(original_lane, our_car, 30):
                our_car.lane_id = original_lane
                self.overtaking_state = None
                self.target_car_id = None
...
def init_cars() -> Dict[int, Car]:
    ...
    cars = {
        0: Car(0, 2, 0.0, 60.0, max_speed=80.0, target_speed=65.0),  # Our car - middle lane
        1: Car(1, 1, rng.uniform(150.0, 250.0), rng.uniform(45.0, 55.0), max_speed=60.0),  # Left lane
        2: Car(2, 2, rng.uniform(200.0, 300.0), rng.uniform(35.0, 45.0), max_speed=50.0),  # Same lane, slower
        3: Car(3, 3, rng.uniform(100.0, 200.0), rng.uniform(50.0, 65.0), max_speed=70.0),  # Right lane
        4: Car(4, 1, rng.uniform(300.0, 400.0), rng.uniform(40.0, 50.0), max_speed=55.0),  # Left lane, ahead
    }
    return cars
...
def update_other_cars(cars: Dict[int, Car], rng: random.Random) -> None:
    ...
    for cid, car in cars.items():
        ...
        speed_change = rng.uniform(-1.0, 1.0)
        car.speed = max(20.0, min(car.speed + speed_change, car.max_speed))
...
def main() -> None:
    ...
    st.markdown("*Advanced B-tree sensor data processing with realistic overtaking*")
    ...
    if "t" not in st.session_state:
        ...
        st.session_state.driver = AutonomousDriver(st.session_state.sensor_processor)
    ...
    with left:
        ...
        if c1.button("▶️ Start", disabled=st.session_state.running and not st.session_state.paused):
            st.session_state.running = True
            st.session_state.paused = False
            
        pause_resume_label = "⏸️ Pause" if not st.session_state.paused else "▶️ Resume"
        ...
        st.write(f"⏱️ **Time:** {st.session_state.t} seconds")
        
        # Decision display
        decision_color = "🟢" if "Cruising" in st.session_state.decision else "🟡" if "Following" in st.session_state.decision else "🔴"
        st.markdown(f"### {decision_color} Decision: {st.session_state.decision}")
        
        # Traffic signal status
        signal = st.session_state.signal
        signal_emoji = "🟢" if signal.state == SignalState.GREEN else "🟡" if signal.state == SignalState.YELLOW else "🔴"
        st.markdown(f"🚦 **Traffic Signal:** {signal_emoji} {signal.get_state_name()}")

        # B-tree data
        st.subheader("🌳 B-tree Data")
        keys = st.session_state.manager.inorder_keys()
        st.code(", ".join(map(str, keys[-20:])) if keys else "(empty)")
        ...
        for cid, car in st.session_state.cars.items():
            car_name = "🚗 Our Car" if cid == 0 else f"🚙 Car {chr(ord('A') + cid - 1)}"
    ...
    with right:
        st.subheader("🛣️ 3-Lane Highway View")
        
        # Create road visualization
        df = pd.DataFrame([
            {
                "car": "🚗" if cid == 0 else f"🚙{chr(ord('A') + cid - 1)}",
                "lane": car.lane_id,
        ...
        road = alt.Chart(df).mark_point(size=150).encode(
            x=alt.X("position:Q", title="Position (m)", scale=alt.Scale(domain=[0, 1000])),
            y=alt.Y("lane:N", title="Lane", sort=[1, 2, 3]),
            color=alt.Color("car:N", scale=alt.Scale(range=["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFEAA7"])),
